...
def get_dataloader(
    ...
    data_loader = data.DataLoader(
        dataset=ImpactSoundDataset(data_path,ignore, split, num_samples, is_augmentation),
...
class SoundClassifier:
    ...
    def offline_train(self, num_epochs=300, lr=0.001):
        ...
        train_loader = get_dataloader(
            data_path=self.data_path,ignore=self.ignore, split="train", is_augmentation=True
        )
        # iter_train_loader = iter(train_loader)
        # train_wav, train_cat = next(iter_train_loader)
        ...
        test_loader = get_dataloader(data_path=self.data_path, split="test")
    ...
    # Load the best model
    def offline_test(self, plot_on=True):
        from sklearn.metrics import accuracy_score, confusion_matrix
